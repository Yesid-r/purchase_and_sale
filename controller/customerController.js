...
export const updateCustomer = async (req, res) => {
    try {
        const { customer_id, name, price_per_bottle } = req.body
        const customer = await Customer.findById(customer_id)
        if (!customer) {
            return res.status(404).json({ success: false, message: "customer not found" })
        } else {

            const customer_update = await Customer.findByIdAndUpdate({ _id: customer_id }, {name: name, price_per_bottle: price_per_bottle})
            res.status(200).json({success:true, message:"customer update", data:customer_update })
        }
    } catch (error) {
